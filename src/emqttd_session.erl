...
-record(session, {
				  ...
				  %% Clean Session Flag
				  clean_sess = true,									%% 是否是临时性session
				  
				  %% ClientId: Identifier of Session
				  client_id   :: binary(),								%% 客户端ID
				  
				  %% Client Pid bind with session
				  client_pid  :: pid(),									%% 对应的emqttd_client进程的Pid
				  ...
				  %% but have not been completely acknowledged.
				  %% Client -> Broker
				  awaiting_rel  :: map(),								%% QOS2类型消息等待PUBREL类型的消息返回确认的存储数据结构
...
handle_cast({puback, PktId}, Session = #session{awaiting_ack = AwaitingAck}) ->
	case maps:find(PktId, AwaitingAck) of
		{ok, TRef} ->
			%% 取消定时器
			cancel_timer(TRef),
			%% 先进行消息的ack操作，然后从消息队列中获得消息然后发送给客户端(如果客户端是离线状态则不作任何处理)
...
%% 发布释放(有保证的交付第2部分)
%% QOS0流程：Publish
%% QOS1流程：Publish -> Pubback
%% QOS2流程：Publish -> Pubrec -> Pubrel -> Pubcomp
...
%% Dispatch Message
%% 发布者通过topic转换找到对应的session进程，此处处理接收到的消息
...
%% 如果客户端连接进程不存在(即客户端离线)，则直接将消息存储到队列中
...
%% Dequeue and Deliver
%%--------------------------------------------------------------------
%% 从消息队列中获得消息然后发送给客户端(如果客户端是离线状态则不作任何处理)

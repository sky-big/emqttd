...
-record(proto_state, {
					  ...
					  will_msg,
					  keepalive,
					  max_clientid_len = ?MAX_CLIENTID_LEN,			%% 客户端ID最大长度
					  session,
...
process(Packet = ?CONNECT_PACKET(Var), State0) ->
	...
	%% 根据连接信息得到最新的emqttd_protocol数据结构
	...
	{ReturnCode1, SessPresent, State3} =
		%% 验证连接信息的正确性(验证MQTT协议版本的正确性和验证客户端ID长度的正确性)
...
process(?PUBACK_PACKET(?PUBREL, PacketId), State = #proto_state{session = Session}) ->
	...
	%% 服务器接收到PUBREL类型的消息，立刻客户端发送PUBCOMP类型消息
...
%% 如果客户端的ID为空，则由服务器生成一个新的ID作为客户端ID

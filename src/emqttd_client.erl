...
-record(client_state, {
					   ...
					   peerhost,					%% 客户端host
					   peerport,					%% 客户端port端口
					   await_recv,					%% 是否等待接收Socket的数据(相当于{active, once})
					   conn_state,					%% 当前连接进程的状态
					   rate_limit,					%% 流量速率
					   parser_fun,					%% 二进制流解析模块emqttd_parser
					   proto_state,					%% emqttd_protocol
					   packet_opts,					%% 消息包的配置参数列表
					   keepalive
					  }).
...
init([OriginConn, MqttEnv]) ->
	...
	%% 组装emqttd_client进程的状态数据机构，然后异步去接收一次Socket数据
	...
	%% 让当前emqttd_client连接进程进入gen_server循环，等待消息的处理
...
handle_info({inet_async, _Sock, _Ref, {ok, Data}}, State) ->
	...
	%% 让await_recv字段置为false，表示先不从Socket接收数据，rate_limit函数里同时允许从Socket异步获得数据
...
%% Socket接收速率的相关问题处理，同时允许从Socket异步获得数据
